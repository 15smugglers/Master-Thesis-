...
outputanalysis<-function(out.dir,out.file,burn,doi.plot=NA,yoi=1:T,pdf.file=NA,P.samples=NA,full.analysis=TRUE,db=NA,dr=NA,U.con=FALSE) {
  ...
  cdir="~/Library/Mobile Documents/com~apple~CloudDocs/Studium/Dissertation MSc/Data/MCMC_Chimp_new_lkd"  #Original like this, still have to do thatcdir=getwd()
  setwd(out.dir)
  ...
  #Reassign "dioceses" here to plot the mean U and mean Z in different subplots 
  
  n_chimps <- length(cil)
  dioc_index <- as.integer(n_chimps/2)+1
  dioc_index <- rep(seq(1:dioc_index),2)
  
  # Dummy example: everyone in different plot 
  for(i in 1:n_chimps){
    cil[[i]]$diocese <- sprintf("Gombe %d",dioc_index[i])
  }
  ...
  #Note: I have to change windows to quartz so that i works on mac! 
  
  if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-mcmc.pdf',sep=''),6,4)}
  ...
  if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-rho-th-p.pdf',sep=''),6,2)}
  ...
  if (is.na(pdf.file)) {quartz()} else {pdf(file=paste(pdf.file,'-beta.pdf',sep=''),paper='a4',6,6)} 
  ...
  if (exists("DOTAU",where=note) && note$DOTAU) {
    ...
    if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-tau.pdf',sep=''),8,6)}
  ...
  if (full.analysis) {
    ...
    if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-Uav.pdf',sep=''),7.8,10)}
    showDOIs(doi.plot=doi.plot,V.mean=U.mean,V.sd=U.sd,cil.plot=cil,B,T,sV=sU,tau.pm.plot=tau.pm)
    if (!is.na(pdf.file)) dev.off()
    
    if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-Zav.pdf',sep=''),7.8,10)}
    ...
    if (!is.na(P.samples)) {
      ...
      if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-pFLdiff.pdf',sep=''),7.8,10)}
    ...
    if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-POconB.pdf',sep=''),40,20)} #8,10
    showDAGcon(B,T,PO.con,years=yoi)
    if (!is.na(pdf.file)) dev.off()
    if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-POconB-tr.pdf',sep=''),40,20)} #8,10
    ...
    if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-POmcmc.pdf',sep=''),6,3)}
    ...
    if (exists("DOSYNTH",where=note) && note$DOSYNTH) {
      ...
      if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-truePO.pdf',sep=''),6,3)}
      showDAGs(B,T,Sstate$h,years=yoi,ecol='red')
      if (!is.na(pdf.file)) dev.off() 
    ...
    if (is.na(pdf.file)) {quartz(10,10)} else {pdf(file=paste(pdf.file,'-names.pdf',sep=''),paper='a4')}
    ...
    if (FALSE) {
      if (is.na(pdf.file)) quartz() #beta traces are a bit of a mess
      ...
      n.yoi=length(yoi)
      if (is.na(pdf.file)) quartz(20,10) #U traces?
