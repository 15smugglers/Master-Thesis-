...
Get.DioDates<-function(la,qnames,note) {
  ...
  bdates=as.data.frame(fread(file=note$bishopdatefile, encoding = "Latin-1", stringsAsFactors=TRUE)) #bascially the BishopDates.csv 
  #bdates=read.csv(file="BishopDates-25-7-22.csv",row.names=1)
  #bdates=read.csv(file="BishopDates-4-11-19.csv",row.names=1)
  #bdates$name<-sub("é","e",bdates$name)
  ...
  # erstellt eine leere Liste die wir befüllen können 
  ...
  #Jetzt wird die pil Liste befüllt 
  for (i in 1:NL) {
    for (j in 1:(la[[i]]$ll)) {
      pin=which(qnames$name.id==la[[i]]$w[j]) #wollen die Liste nur mit la$w Werten 
  ...
  #BIl liefert dann die Grundlage für CIL! 
  #Retrieves the dioceses and finds the unique names and the total number of uniques dioceses
  bil=pil
...
plot.dio.dates<-function(pil,dio.names,file=NA,ac.min=0,packed=FALSE,note=list()) {
  ...
  if (packed) {
    ...
    par(mfrow=c(nr<-ceiling(n.doi.plot/3),nc<-3),oma=c(4,4,2,2)) #ceiling <- runden auf die nächst höhere Zahl auf 
  }
  
  dioceses=unlist(lapply(pil,function(x){x$diocese}))
  
  if (!is.na(file)) pdf(file,8,5) #Plot as a pdf 
...
#Reads which bishop was when in office (who[d,t])
#Calculates how long he already is in office (tio[d,t])
#Assigns a rank to each dioceses base on the time in office(drk[d,tk])
...
makejkdata<-function(on=1122,off=1128,takebishops=TRUE,include.nodates=FALSE,include.singletons=FALSE,remove.badbishops=FALSE) {
  ...
  pnames=as.data.frame(fread("person name.txt", encoding = "Latin-1", stringsAsFactors=TRUE))
  ...
  z4n<-pnames[,c('person name ID')] 
  
  #a list of name-ids (not row #s!) corresponding to Bishops
  if (takebishops) {
    #select the bishops
    rw<-grep(c("^bishop"),pnames[,c('name third element')],ignore.case=TRUE)
    bip<-pnames[,c('person name ID')][rw]
