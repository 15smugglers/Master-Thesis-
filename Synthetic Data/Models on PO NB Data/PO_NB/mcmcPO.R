mcmcPO<-function(note) {
  ...
  if (note$NEWRUN) {
    ...
    #Create a object here where we can store the log likleihood for all 
    #Want to create the input matrix for the waic function which is NxJ 
    #N is the number of lists that we are looking at and J is the number of sweeps
    lkd.for.waic <- c(0)
    ...
    # command that out and give the data directly 
    #D=makedata(note,B,E,T)
    #cla=D$cla; cil=D$cil; doi=D$doi
    if(note$gender=='male') {
      cla=scla; cil=scil; doi=note$doi
    }
    if(note$gender=='female'){
      cla=scla; cil=scil; doi=note$doi 
    }
    else{
      print('Inputed wrong gender')
    }
    
    #Apply the restriction function here 
    finished=FALSE
    print(sprintf("Loaded num lists = %d, num bishops = %d",length(cla),length(cil)))
    print("trimming to [B,E] interval")
    while(!finished) {
      restricted=restrict(La=cla,bil=cil,note$B,note$E,boi=1:length(cil),doi=note$do,b_min_list=1,max_list_length=Inf,rule=note$selectlists)
    ...
    ll=sort(sapply(cla,function(x) x$ll),decreasing=TRUE)
    ...
    obp=dBprior(state$beta) #function is defined above! 
  ...
  for (j in start:J) {
    ...
    if (note$DOBETA) {
      ...
      stb<-system.time({
        for (r in 1:DB) {
          ...
          if (no.change.h) {
            MHR=new.bp-obp
            new.ll=oll #if exactly the same than we keep the likelihood we had in the run before 
            new.ll.tot=oll.tot
          } else {
            if (state$p>0 || noconflict2(new.h,state$y2l,cla)$outcome || note$model=='prior') { #state$p>0 || ??
              new.ll=log.lkd.par(new.h,cla,state$y2l,years=1:T,p=state$p,model=note$model,q=state$q,cl=cl,f0=f0)  #we calculate the new likelihood using the new h! 
              new.ll.tot=sum(unlist(new.ll))
              MHR=new.ll.tot-oll.tot+new.bp-obp #we calulcate the metropolis hastings ratio here. Simply new/old Likelihood*prior
            } else {
              MHR=-Inf #then we won't accept 
            }
          }
    ...
    for (trur in 1:note$rhothetaU.wrap.reps) { #according to Jeffs paper: Proposals for rho and theta (which are in [0; 1]) are prior draws with a Hastings ratio which does not involve the final likelihood factor in Eqn 17.
    ...
    if (note$model!='prior') {
      ...
      lkd.for.waic <- c(lkd.for.waic,check.ll)
      if (!isTRUE(all.equal(check.ll,oll))) stop('llk not updating correctly') #is that where we can take the likelihood from?
