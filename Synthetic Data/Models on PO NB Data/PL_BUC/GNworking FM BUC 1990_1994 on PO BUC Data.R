...
note$RUNDIR="C:/Users/fimaass/Downloads/Synthetic Data/Models on PO NB Data/PL_BUC"
...
note$savefile='example-FT_theta.RData'  #write MCMC samples to savefile
...
B <- note$B;E <- note$E
#####
D <- get_data_list_2(cla=scla,cil=scil,V=V,B=B,E=E) #we dont use the V part so might as well just leave it out. 
...
beta_log_alpha <- function(oldbeta,D,i,oldlambda,sigmabeta,oldll,B=B,Np){
  ...
  newll <- PL_log_lik(D=D,lambda=oldlambda,beta=newbeta,B=B) #are we really updating the likelihood here? We dont change the likleihood #likelihood is calculated with all included! 
...
sigma_log_alpha <- function(oldsigma,wsigma,oldlambda,oldtheta,a,b,tt,N){
  ...
  if(newsigma <=0){logalpha <- log(0)} # reject if sigma is less than zero
    ...
    oldp1 <- dmvnorm(t(oldlambdap[,2:tt]-oldtheta*oldlambdap[,1:(tt-1)]),sigma=oldsigma^2*N,log=T) # 2--tt columns
    oldp <- oldp0+sum(oldp1)
    
    newp0 <- dmvnorm(oldlambdap[,1],sigma=newsigma^2/(1-oldtheta^2)*N,log=T)
    newp1 <- dmvnorm(t(oldlambdap[,2:tt]-oldtheta*oldlambdap[,1:(tt-1)]),sigma=newsigma^2*N,log=T)
...
lambda_log_alpha <- function(oldll,oldlambda,oldtheta,oldsigma,D,oldbeta,sigmalambda,B,i,t,tt,N){
  ...
  newlambda[,t] <- newlambda[,t]-mean(newlambda[,t]) ### center the lambdas
  ...
  if(t==1){
    logalpha <- logalpha+dmvnorm(newlambdap[,t],sigma=oldsigma^2/(1-oldtheta^2)*N,log=T)-dmvnorm(oldlambdap[,t],sigma=oldsigma^2/(1-oldtheta^2)*N,log=T)+dmvnorm(newlambdap[,(t+1)]-oldtheta*newlambdap[,t],sigma=oldsigma^2*N,log=T)-dmvnorm(oldlambdap[,(t+1)]-oldtheta*oldlambdap[,t],sigma=oldsigma^2*N,log=T)
  }
  else{
    if(t==tt){
      logalpha <- logalpha+dmvnorm(newlambdap[,t]-oldtheta*newlambdap[,t-1],sigma=oldsigma^2*N,log=T)-dmvnorm(oldlambdap[,t]-oldtheta*oldlambdap[,t-1],sigma=oldsigma^2*N,log=T)
    }
    if(t!=tt){
      logalpha <- logalpha +dmvnorm(newlambdap[,t]-oldtheta*newlambdap[,t-1],sigma=oldsigma^2*N,log=T)-dmvnorm(oldlambdap[,t]-oldtheta*oldlambdap[,t-1],sigma=oldsigma^2*N,log=T) + dmvnorm(newlambdap[,t+1]-oldtheta*newlambdap[,t],sigma=oldsigma^2*N,log=T)-dmvnorm(oldlambdap[,t+1]-oldtheta*oldlambdap[,t],sigma=oldsigma^2*N,log=T)
    }
  }
...
X <- PL_MCMC(D=D,M=M,wsigma=0.1,wtheta=0.1,sigmabeta=1.0,sigmalambda=0.5,SS=SS,B=B,E=E,d=10,a=0.5,b=0.5,theta0=thetahat,sigma0=sigmahat,beta0=betahat,lambda0=lambdahat)
