#do restriction before calling this 
makeparameters<-function(note,cla,cil,B=note$B,E=note$E,Sstate=NA) {
  ...
  active=!is.na(rank) 
  MA=max(apply(active,2,sum)) #maximum number of bishops ever present
  ...
  DB=max(rank,na.rm=TRUE) #maximun entry in rank 
  ...
  if (note$STARTSTATE=='disordered') {
    ...
    #generate empty order by reverse ordering cols of U
    Sig=matrix(state$rho,NF,NF); diag(Sig)=1;
    state$U=array(dim=c(NB,NF,T))
    i=1:NB
    state$U[i,,1]=mvrnorm(NB,rep(0,NF),Sig)
    if (T>1) {
      for (t in 2:T){
        i=i[NB:1]
        state$U[,,t]=state$U[i,,1] 
      }
    }
    for (t in 1:T) {for (b in 1:NB) {if (!active[b,t]) state$U[b,,t]<-NA}}
    ...
    # is.good=FALSE;
    # while (!is.good) {
    #   state$U=rUprior(active,NF,T,theta=0,rho=0) 
    #   #the probability for an order between any pair of bishops is 1/2^NF so mostly no-order
    #   state$Z=ComputeZ(years=1:T,bishops=1:NB,state$U,rank,state$beta)
    #   state$h<-ZtoPO(state$Z,active,B,T,display.Z=note$DRAW)
